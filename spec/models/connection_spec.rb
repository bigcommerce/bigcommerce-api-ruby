...
shared_examples_for "request method accepting optional params and headers" do
  it "sends request with no params" do
    connection.should_receive(:request).once.with(http_method, path, nil, nil, {})
    connection.send(http_method, path)
  end

  it "sends request with params" do
    connection.should_receive(:request).once.with(http_method, path, nil, {:page => 3}, {})
    connection.send(http_method, path, {:page => 3})
  end

  it "sends request with headers" do
    connection.should_receive(:request).once.with(http_method, "/orders", nil, nil, {'Some-Header' => 'abc'})
    connection.send(http_method, path, nil, {'Some-Header' => 'abc'})
  end
end
  ...
  [:get, :post, :put, :delete].each do |http_method|
    ...
    describe '#request' do
      context "when #{http_method.to_s.upcase} request" do
        ...
        let(:args) { [http_method, path, body, params, headers] }
        let(:body) { nil }
        let(:params) { {} }
        let(:headers) { {} }
